				...
				<div class="pd-20 card-box mb-30">
					...
					<form method="post" action="">
						...
						if (!isset($_SESSION['data']) && empty($_SESSION['data'])) {
							...
							if ($query->rowCount() > 0) {
								foreach ($results as $result) {
									$_SESSION['descontoc'] = $result->descontoc;
						?>
				...
				<div class="pd-20 card-box mb-30">
					<div class="clearfix">
						...
						<div class="pull-right">
							<a><button class="btn btn-primary" rel="content-y" id="action_take" data-toggle="modal" role="button" data-target="#modal-view-event-add">Adicionar Artigo</a></button>
						</div>
					</div>
						...
						<table class="data-table table stripe hover nowrap">
							...
							<tbody>
								...
								<tr>																					
										...
										<td><?php 
											    $ValorLinha= number_format( $row['precol'], 2, ',', '.' );
												echo $ValorLinha; ?>
										</td>
										<td>
											<div class="table-actions">

												<a href="editarlinha.php?edit=<?php echo $row['cod_onlinha']; ?>" data-color="#265ed7"><i class="icon-copy dw dw-edit2"></i></a>
												<form action="eliminarlinha.php" method="post"> <input type="hidden" name="cod_onlinha" value="<?php echo $row['cod_onlinha']; ?>"><button type="submit" style="border: none !important;" data-color="#e95959"><i class="icon-copy dw dw-delete-3" style="font-size: 18px;"></i></button></form>
											</div>
										</td>													
								</tr>
							<?php } ?>
								...
								<form id="add-event">
									<div class="modal-body">
										...
										<div class="form-group">
											...
											<input class="form-control" name="quantidadelinha" id="quantidadelinha" value="1" type="number" min="1" required/>
										</div>
										<div class="form-group">
											<label>Desconto (%)</label>
											<input class="form-control" name="desconto" id="desconto" value="0" type="number" min="0" max="25" required/>
										</div>
										<div class="form-group">
											<label>IVA</label>
										...
										<div class="form-group">
											<label>Stock</label>
											<input class="form-control" name="stock" id="stock" value="" type="text" readonly/>
										</div>
...
<script>
	...
	request.onreadystatechange = function() {
		if (request.readyState === 4 && request.status === 200) {
			...
			document.getElementById("stock").value = products[0].stock;
	...
	function handlesubmit() {
		...
		if (quantidadelinha > 0 && desconto >= 0 && desconto <= 25) {
			...
			formData.append("codartigo", codartigo);
			formData.append("product_name", product_name);	
			formData.append("Unidade", Unidade);			
			formData.append("quantidadelinha", quantidadelinha);
